...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  if (data.credentials.password === data.credentials.password_confirmation) {
	    api.signUp(data).done(ui.signUpSuccess, ui.hideForm(form), $('#password-message').hide()).fail(ui.failure);
	  } else {
	...
	var onSignIn = function onSignIn(event) {
	  ...
	  api.signIn(data).done(ui.signInSuccess, ui.hideForm(form)).fail(ui.failure);
	};
	...
	var onChangePassword = function onChangePassword(event) {
	  ...
	  api.changePassword(data).done(ui.changePasswordSuccess, ui.hideForm(form)).fail(ui.failure);
	};
	...
	var onGetGame = function onGetGame(event) {
	  ...
	  api.getGame(data).done(ui.getGameSuccess, ui.hideForm(form)).fail(ui.failure);
	};
	...
	var onStartGame = function onStartGame(event) {
	  ...
	  api.startGame().done(ui.startGameSuccess, ui.hideForm(form)).fail(ui.failure);
	};

	var buildUpdateDataFromClick = function buildUpdateDataFromClick(event) {
	...
	var updateStandardGame = function updateStandardGame() {
	  var index = $(event.target).data('id');

	  if (!app.user.currentGame.over && logic.isValidMove(index)) {
	    var indexWins = $('[data-id=' + index + ']').data('win-conditions');
	    logic.incrementGameConditions(indexWins, app.player);

	    var data = buildUpdateDataFromClick(event);
	...
	var updateTacticoGame = function updateTacticoGame(event) {
	  var index = $(event.target).data('id');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var renderGameBoard = function renderGameBoard(game) {
	  clearGameBoard();
	  var cellArray = game.cells;
	...
	var celebration = function celebration() {
	  if (logic.isGameWin()) {
	    ...
	    $('.turn').hide();
	    var _game = {};
	    _game.cells = ['d', 'l', 'l', 'l', 'd', 'l', 'l', 'l', 'd'];
	    var _game2 = {};
	    _game2.cells = ['l', 'd', 'l', 'l', 'd', 'l', 'l', 'd', 'l'];
	    var _game3 = {};
	    _game3.cells = ['l', 'l', 'd', 'l', 'd', 'l', 'd', 'l', 'l'];
	    var game4 = {};
	    game4.cells = ['l', 'l', 'l', 'd', 'd', 'd', 'l', 'l', 'l'];

	    var _timeInterval = 250;
	    var _cycles = 6;
	    var _frames = 4;
	    for (var _i = 1; _i < _cycles; _i++) {
	      setTimeout(renderGameBoard, _timeInterval * (_frames * _i - 3), _game);
	      setTimeout(renderGameBoard, _timeInterval * (_frames * _i - 2), _game2);
	      setTimeout(renderGameBoard, _timeInterval * (_frames * _i - 1), _game3);
	      setTimeout(renderGameBoard, _timeInterval * (_frames * _i), game4);
	    }
	    setTimeout(renderGameBoard, _timeInterval * (_frames * _cycles), app.user.currentGame);
	  }
	};
	...
	var updatePlayerTurn = function updatePlayerTurn() {
	  $('.turn').toggle();
	};

	var hideForm = function hideForm(formId) {
	  $(formId).hide();
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var clearGameConditions = function clearGameConditions() {
	  for (var condition in winConditions) {
	    winConditions[condition] = 0;
	  }
	};

	var incrementGameConditions = function incrementGameConditions(winCondArray, player) {
	  var increment = 0;
	  if (player === 'x') {
	    increment = 1;
	  } else if (player === 'o') {
	  ...
	  for (var i = 0; i < winCondArray.length; i++) {
	    var condition = winCondArray[i];
	    app.winConditions[condition] += increment;
	  }
	};

	var setGameConditions = function setGameConditions() {
	  app.player = 'x';
	  clearGameConditions();
	  var cellArray = app.user.currentGame.cells;
	  app.winConditions = winConditions;

	  for (var i = 0; i < cellArray.length; i++) {
	    var winCondArray = $('[data-id=' + i + ']').data('win-conditions');
	    incrementGameConditions(winCondArray, cellArray[i]);
	  }
	};

	var isValidMove = function isValidMove(index) {
	  var boardArray = app.user.currentGame.cells;
	  if (boardArray[index] === '') {
	    return true;
	  } else {
	    return false;
	...
	var isGameWin = function isGameWin() {
	  var gameConditions = app.winConditions;

	  for (var condition in gameConditions) {
	...
	var isGameTie = function isGameTie() {
	  var boardArray = app.user.currentGame.cells;
	  ...
	  if (isGameWin()) {
	    ...
	    for (var _i = 0; _i < boardArray.length; _i++) {
	      if (boardArray[_i] === '') {
	        if (++count === 2) {
	          return false;
	        }
	...
	module.exports = {
	  ...
	  setGameConditions: setGameConditions,
	  incrementGameConditions: incrementGameConditions
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
